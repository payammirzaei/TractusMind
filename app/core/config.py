...
class Settings(pydantic_settings.BaseSettings):
    ...
    cors_origins: str = ""
    request_max_body_bytes: int = pydantic.Field(default=65_536, ge=4_096, le=10_485_760)
    max_concurrent_requests: int = pydantic.Field(default=64, ge=1, le=10_000)
    rate_limit_requests: int = pydantic.Field(default=120, ge=1, le=100_000)
    rate_limit_window_seconds: float = pydantic.Field(default=60.0, gt=0.0, le=3_600.0)
    ...
    llm_timeout_seconds: float = pydantic.Field(default=60.0, gt=0.0, le=300.0)
    llm_max_attempts: int = pydantic.Field(default=3, ge=1, le=6)
    llm_temperature: float = pydantic.Field(default=0.0, ge=0.0, le=2.0)
    ...
    oidc_http_timeout_seconds: float = pydantic.Field(default=10.0, gt=0.0, le=60.0)
    oidc_cache_ttl_seconds: int = pydantic.Field(default=3_600, ge=30, le=86_400)

    provider_retry_base_seconds: float = pydantic.Field(default=0.5, ge=0.0, le=30.0)
    provider_retry_max_seconds: float = pydantic.Field(default=8.0, ge=0.1, le=120.0)
    provider_circuit_failure_threshold: int = pydantic.Field(default=3, ge=1, le=20)
    provider_circuit_cooldown_seconds: float = pydantic.Field(default=30.0, gt=0.0, le=600.0)

    embedding_model: str = "BAAI/bge-small-en-v1.5"
    embedding_batch_size: int = pydantic.Field(default=32, ge=1, le=256)
    sparse_embedding_model: str = "Qdrant/bm25"
    sparse_embedding_batch_size: int = pydantic.Field(default=32, ge=1, le=256)
    hybrid_prefetch_k: int = pydantic.Field(default=40, ge=2, le=500)
    reranker_model: str = "Xenova/ms-marco-MiniLM-L-6-v2"
    reranker_batch_size: int = pydantic.Field(default=32, ge=1, le=128)
    debug_exact_k: int = pydantic.Field(default=30, ge=1, le=200)
    debug_rrf_k: int = pydantic.Field(default=60, ge=1, le=500)
    ...
    worker_metrics_port: int = pydantic.Field(default=9_101, ge=0, le=65_535)
    scheduler_metrics_port: int = pydantic.Field(default=9_102, ge=0, le=65_535)
    otel_traces_endpoint: str | None = None
    otel_service_name: str = "tractusmind-api"
    otel_sample_ratio: float = pydantic.Field(default=1.0, ge=0.0, le=1.0)
    ...
    retrieval_top_k: int = pydantic.Field(default=20, ge=1, le=100)
    rerank_top_k: int = pydantic.Field(default=6, ge=1, le=50)
    minimum_relevance_score: float | None = pydantic.Field(default=None, ge=-100.0, le=100.0)

    @pydantic.model_validator(mode="after")
    def load_secret_files(self) -> typing.Self:
